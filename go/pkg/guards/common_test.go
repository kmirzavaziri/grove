...
func (s *CommonSuite) TestType() {
	for _, tCase := range []testCase{
		...
		for _, typeName := range gex.Types {
			errMsg, err := guards.New(guards.Type{TypeName: typeName}).Guard(tCase.d)
...
func (s *CommonSuite) TestNotEmpty() {
	testCases{
		{t: "nil", d: gex.Nil(), v: guards.Empty{}, errMsg: ""},

		{t: "string - empty", d: gex.Str(""), v: guards.Empty{}, errMsg: ""},
		{t: "string - not empty", d: gex.Str("test"), v: guards.Empty{}, errMsg: "Guard failed"},

		{t: "list - empty", d: gex.List(), v: guards.Empty{}, errMsg: ""},
		{
			t: "list - not empty",
			d: gex.List(gex.Str("item")), v: guards.Empty{}, errMsg: "Guard failed",
		},

		{
			t: "map - empty",
			d: gex.Map(map[string]*gex.Value{}), v: guards.Empty{}, errMsg: "",
		},
		{
			t:      "map - not empty",
		...
		{t: "number - zero", d: gex.Num(0), v: guards.Empty{}, errMsg: "Guard failed"},
		{t: "number - negative", d: gex.Num(-42), v: guards.Empty{}, errMsg: "Guard failed"},
		{t: "number - positive", d: gex.Num(42), v: guards.Empty{}, errMsg: "Guard failed"},

		{t: "boolean - false", d: gex.Bool(false), v: guards.Empty{}, errMsg: "Guard failed"},
		{t: "boolean - true", d: gex.Bool(true), v: guards.Empty{}, errMsg: "Guard failed"},
	}.run(s.Suite)
}
...
func (s *CommonSuite) TestErrorMessage() {
	testCases{
		...
		},
		{
			...
			d: gex.Str("text"),
			v: guards.Not{
				Guard: guards.New(guards.Type{TypeName: gex.TypeString}),
				Err:   "M - ${value}",
			},
			errMsg: "M - text",
		},
		{
			...
			v: guards.And{
				Guards: []guards.Guard{
					guards.New(guards.HasPrefix{Prefix: "t"}),
					guards.New(guards.HasSuffix{
						Suffix: "f",
		...
		},
		{
			...
			v: guards.Or{
				Guards: []guards.Guard{
					guards.New(guards.HasPrefix{Prefix: "a"}),
					guards.New(guards.HasSuffix{Suffix: "z"}),
				},
				Err: "M - ${value}",
			},
			errMsg: "M - text",
		...
		},
		{
			...
			d: gex.Str("text"),
			v: guards.All{
				Guard:   guards.New(guards.HasPrefix{Prefix: "a"}),
				ErrType: "M - ${value}",
			},
			errMsg: "M - text",
		},
		{
			...
			v: guards.All{
				Guard: guards.New(guards.HasPrefix{
					Prefix: "a",
